...
(defun current-room (state)
  (equal (room-index (context-current-room state)) '(0 0)))

(defconstant +feelings+
  `((dread . ((excitement . "...")
               (condition . ((#'check-status . (((foreboding) . 0.2)
                                                 ((death) . 0.2)
                                                 ((grime) . 0.2)
                                                 ((dark) . 0.4)))
...
(defconstant +event-classes+
  `((feeling . ((name . "A feeling of ~S falls upon you~S")
                (parameters . (name excitement))
                (variants . ,+feelings+)))
    (status-effect . ((name . "You feel ~S~S")
                       (parameters . (name excitement))
                       (variants . ,+status-effects+)))
    (apparation . ((name . "A ~S appeared ~S from ~S")
                    (parameters . (name excitement))
     ...
     ,(map 'list
           (lambda (event-class)
             (let ((variants (cdr (assoc 'variants (cdr event-class))))
               ...
               (map 'list
                    (lambda (variant)
                      `(,(car variant) . ,(list `(name . (format nil ,name
                                                              ,@(map 'list
                                                                     (lambda (key) (assoc key (cdr variant)))
                                                                     parameters)))
                                             (assoc 'condition variant)
                                             (assoc 'effect variant))))
                    variants)))
