...
(defparameter +events-per-tick+ 3)
(defparameter +ticks-per-year+ 30000)
...
(defparameter +status-effects+
  `((dizzy . ((excitement . "...")
              (condition . ((,(lambda (x) T) . ((nil . 1.0)))))
              (effect . ((,(lambda (x) T) . ((nil . 1.0)))))))
    (confused . ((excitement . "...?")
                 (condition . ((,(lambda (x) T) . ((nil . 1.0)))))
                 (effect . ((,(lambda (x) T) . ((nil . 1.0)))))))
...
(defun simulate-event (state event)
  ...
  (format t "~a~%~a~%" (cdr (assoc 'name (cdr event))) (cdr (assoc 'description (cdr event))))
